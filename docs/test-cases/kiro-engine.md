...
- 内容碰撞：stdout/stderr 包含权限或上下文诊断关键词时仍按退出码处理、不根据自然语言误报，由 `internal/engine/kiro_test.go` 的确定性单测覆盖。
- 权限 YAML 保留已有规则/注释/文件模式、幂等去重、无效 YAML 停止执行，v3 精确 argv、完整 stdout、非零退出只取 stderr（空 stderr 时只报退出码）、临时文件不落盘和子进程组清理，均由 `internal/engine/kiro_test.go` 的确定性假二进制单测覆盖；真实冒烟不重复制造不稳定故障。
...
TC-001、TC-002、TC-003 是**可选外部兼容性测试**，不属于日常确定性回归。它们会真实调用 Kiro、消耗 credits、创建当前用户可见的 session，并在当前 Kiro profile 的 `settings/permissions.yaml` 中永久确保存在 `all/allow`；这是经用户确认的 Conduct 既定行为，也会影响之后手动启动的 Kiro。仅在需要确认当前 Kiro CLI 版本仍与 Conduct 兼容时执行。
...
每个会写 Conduct store 的用例都引用 [atomic-conduct-test.sh](./atomic-conduct-test.sh)：它在单个 shell 内注册 trap、把 Conduct 的 `HOME` 重定向到临时目录，并比较真实 `~/.conduct/workflows` / `runs` 前后快照。用例把 `KIRO_HOME` 显式指回真实用户的 `.kiro`，并在临时 `PATH` 中安装同名薄包装器为 Kiro 恢复真实 `HOME`；这样只隔离 Conduct 的 `~/.conduct`，不会隔离或复制当前用户的 Kiro profile。`KIRO_HOME` 同时确保 Conduct 合并权限和 Kiro 读取权限指向同一 profile。
...
- **步骤**：完整复制执行：
  ...
  conduct_test_setup
  export KIRO_HOME="$CONDUCT_TEST_REAL_HOME/.kiro"
  export REAL_KIRO_CLI
  cat > "$WORK/fakebin/kiro-cli" <<'SH'
  #!/usr/bin/env bash
  # Conduct 的 HOME 仅用于隔离 ~/.conduct；Kiro 仍复用当前用户的登录态、设置与 session。
  exec env HOME="$CONDUCT_TEST_REAL_HOME" KIRO_HOME="$KIRO_HOME" "$REAL_KIRO_CLI" "$@"
  SH
...
- **清理**：脚本 trap 恢复环境、删除临时 Conduct store 与项目，并验证真实 Conduct store 零差异；当前用户的 Kiro profile 保留新 session 和 `all/allow` 权限，这是被测副作用。

### TC-002 shell/read/write、图片与项目配置

- **目的**：验证 Kiro v3 的持久 `all/allow` 权限允许隔离 cwd 内的 shell/read/write 无需审批；能识别 prompt 中本地 PNG 的绝对路径与可辨识文字；项目 `.kiro/steering` 生效，同时复用当前用户的 Kiro profile。
- **前置**：已完成〈环境准备〉，当前用户的 Kiro CLI 已登录。
- **步骤**：完整复制执行：
  ...
  PY
  IMAGE_PATH="$PROJECT/kiro-7319.png" PROJECT="$PROJECT" python3 -c 'import json,os; p=os.environ["IMAGE_PATH"]; result=os.path.join(os.environ["PROJECT"],"result.txt"); prompt=f"必须依次使用 shell 执行 pwd、read 读取本地绝对路径 {p} 的 PNG 并识别其中的文字、write 在绝对路径 {result} 创建文件（内容精确为 KIRO-WRITE-OK），再用 shell 验证该绝对路径；最后回答必须包含图片文字和项目配置 sentinel。"; print(json.dumps({"nodes":[{"id":"START"},{"id":"tools","displayName":"Kiro tools","engine":"kiro","promptTemplate":prompt},{"id":"END"}],"edges":[{"from":"START","to":"tools"},{"from":"tools","to":"END"}]}))' | "$CONDUCT" workflow create kiro-tools --definition >/dev/null
...
- **清理**：脚本 trap 删除隔离 Conduct store 与项目并验证真实 Conduct store 零差异；当前用户的 Kiro profile 保留 session 与 `all/allow` 权限副作用。
...
- **清理**：atomic shell 的 trap 恢复环境、删除临时 Conduct store 并验证真实 Conduct store 零差异；当前用户的 Kiro profile 保留 session 与 `all/allow` 权限。
