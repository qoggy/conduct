...
- **engine 取值与各自 engineConfig 允许字段**：antigravity=model；claude-code=model + effort；codex/qoder=model + reasoningEffort（各自 effort/reasoningEffort 的枚举）
...
**How to apply:** 新增 / 改任何命令的 Short / Long / flag 描述、或文档里提到上述通用常量时，照权威来源的措辞抄。engine 名与 effort / reasoningEffort 枚举优先从 `internal/engine` 能力表动态生成、避免静态文案漂移（见 [[coding-help-for-llm]]）；改动波及 docs/specs 时同步（见 [[coding-spec-sync]]）。
