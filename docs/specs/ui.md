...
- 每个 agent 节点一张**紧凑卡片**（`.fnode`）：**左侧引擎品牌 icon + displayName**；节点间以带箭头的连线表达边（`from → to`）。
...
**右栏 · 检查器**（选中节点的完整配置；沿用现有实现，仅去掉循环控件）：`id`（可编辑，即时正则校验；失焦 / 回车提交时**级联改名**——把所有边的 `from/to` 与各节点模板里的 `{{旧id}}` 引用一并换成新 id，不留悬空引用；重名 / 保留名 / 非法一律拒绝回滚。CLI 侧对等命令是 `workflow node set --id`，共用同一套 `workflow.RenameNodeID` 级联语义，符合「UI 无独占能力」不变量）、`displayName`、`engine` 下拉（数据源 `GET /api/engines`，带引擎品牌 icon）、`engineConfig` 按判别联合**条件渲染**（claude-code → model + effort；qoder → model + reasoningEffort；codex → model + reasoningEffort；antigravity → 仅 model；该引擎没有的字段不渲染、不配解释文案；model 是可手输自由文本，`modelValues` 非空时挂建议下拉、非白名单）、**提示词编辑器**（markdown 源码 + 语法高亮 + `{{占位符}}` 高亮，全文可见可编辑，超高内部滚动、附最大化覆盖层；占位符入口 hover 下拉：sys 变量 + 当前定义内**祖先** agent 节点 id，点击复制）。**去掉** evaluator / redoTarget / loopCount 相关的一切控件与聚焦面板。故 `custom-select` / `engines` / `prompt-editor` / `code-editor` 等组件几乎不动。
...
- **右栏节点详情（未选中节点时整列不显示、画布在整行内居中；点画布节点才展开，再次点该节点或点画布空白即取消选中、回到居中态）**：展开该节点的 **engine · model · tokens · 耗时**（`engineConfig` 声明值在详情顶部，不占画布）+ 若该节点回报了会话 id，附**会话 id + 回放命令**（claude-code→`claude -r`、codex→`codex resume`、qoder→`qodercli -r`、antigravity→`agy --conversation`，未知引擎只显 id；与 `run show --trace` 一致，**会话 id 与回放命令分两行、各自可复制**）+ **输入 / 输出（产物）全文**（`--trace` 深度）。**输入 / 输出各是一个默认折叠的可点开 Markdown 富文本块**（`marked` 解析、Prism 高亮 fenced code、DOMPurify 消毒）；节点 id 收在此面板（`.idchip`）。选中节点才构建详情 DOM，输入 / 输出体再延迟到展开对应折叠块才渲染（trace 单行可达 MB 级）——**延迟渲染是唯一允许的性能手段，展开时渲染完整全文、绝不截断**。
...
- **主题预留**：全部颜色经 `:root` CSS 变量，当前只发 light 值；未来 dark 仅加一段 `[data-theme=dark]` 变量覆盖。
- **语言切换（已实现）**：全部可见产品文案进入 key 集合一致的 `zh-CN` / `en` 字典；顶栏提供“跟随环境 / 中文 / English”，复用 CLI 的 `settings.language` > locale 环境变量 > 英文解析结果。服务端领域错误使用稳定错误码 + 参数，由页面本地化；底层技术详情仍固定英文。完整规则见 [i18n.md](./i18n.md)。
...
| `GET /api/runs/{id}?trace=1` | `run show <id> --json --trace` | 每条含 input / output / error **全文** + `startedAt` / `endedAt`；刷新按钮全量重取（无 `since` 增量，见〈监控机制〉） |
...
| `GET /api/engines` | （无 CLI 命令：只读信息性端点豁免） | 注册引擎 + 能力表（`allowsModel` / `effortField` / `effortValues` / `modelValues`），检查器引擎、effort 与 model 建议下拉的数据源；能力表缺失时以 `capability:null` 表达 |
...
| `GET /api/settings` | （无 CLI 命令：全局配置基础设施豁免） | 返回显式 `language`（跟随环境时为 `null`）与当前 `resolvedLanguage`；损坏设置返回固定英文技术诊断，不降级 |
| `PATCH /api/settings` | （无 CLI 命令：全局配置基础设施豁免） | 只修改/删除 `settings.json` 的 `language` 并保留其它属性；支持“跟随环境 / 中文 / English”，完整 schema 见 [i18n.md](./i18n.md) |
...
1. **【服务端直读·无需新命令】`GET /api/engines`**——检查器引擎 / effort 下拉和 model 建议下拉的数据源，由 UI 服务端 handler 进程内组合 `engine.RegisteredNames()` + `engine.Capability()` 生成，**不新增 `conduct engine list` 命令**。只读信息性端点，作为「无独占能力」不变量的显式豁免。不受本次模型改造影响。
...
9. **【已实现】中英文界面与全局语言设置**——已实现 `GET/PATCH /api/settings`、key 集合一致的 `zh-CN` / `en` 字典、顶栏三选项、动态 `<html lang>`、稳定领域错误码与前端本地化；损坏设置固定英文 fail-loud，完整行为见 [i18n.md](./i18n.md)。
...
└── assets/             # 内嵌前端（index.html / *.js（含图编辑器 editor.js、DAG 进度 run-detail.js）/ style.css / 中英文文案字典 / vendor/）
