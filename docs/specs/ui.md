...
- 表格列 `名称 | 节点流 | 最近修改`，与 CLI 表格同列（随 cli-authoring.md〈workflow list〉修订：节点列展示节点 id 流、移除引擎列）；**行序按 `最近修改` 倒序**（`updatedAt` 降序，相同再按名字升序兜底——与 CLI `workflow list`、`run list` 的时间倒序同源同序，最近编辑的浮到顶）。节点流以中性 chip 呈现（`plan › code › test › review`），超过 6 个折叠为前 6 个 + `+N`。另加 `运行中` 徽标列（该 workflow 下 `status=running` 的 run 数，来自 run 表 join——规格〈数据模型〉明示支持此聚合，等价 `run list --json` 过滤）。行点击进入编辑器。
- **行内操作**：**运行**（打开启动弹窗，见下）· **改名**（弹窗，= `rename`，文案明示「已有运行记录保留旧名」）· **复制**（弹窗，= `copy`，新名输入预填 `<原名>-copy`，文案明示「复制定义主体（节点），生成一份新名称的工作流；原工作流与其运行记录不受影响」；目标重名 409、非法名 400）· **删除**（确认弹窗，= `delete --yes`——UI 弹窗承担 CLI 交互式二次确认的职责；文案明示「运行记录不受影响」）。
...
- `engine` 下拉（数据源 `GET /api/engines` 能力表：当前 claude-code / antigravity / qoder / codex）；选项与左栏节点卡片均带**引擎品牌 icon**（icon 资产随前端内嵌，不引外链）；
- `engineConfig` 按判别联合**条件渲染**——claude-code → model 输入 + effort 下拉（low/medium/high/xhigh/max/ultracode/auto）；qoder → model 输入 + reasoningEffort 下拉（disabled/off/none/low/medium/high/xhigh/max）；codex → model 输入 + reasoningEffort 下拉（low/medium/high/xhigh）；antigravity → 仅 model 输入。**该引擎没有的字段就不渲染，不配解释文案**。model 字段始终是可手输的自由文本输入框，placeholder「留空使用引擎默认模型」；当 `/api/engines` 的 `capability.modelValues` 非空时，在输入框上挂同款自定义建议下拉（claude-code：sonnet / opus / fable；qoder：Auto / Ultimate / Performance / Efficient / Lite），点击建议只负责填入文本，**不是白名单**；留空表示不设置 model，手输非空值不受建议项限制。antigravity / codex 当前未登记建议值，显示普通输入框；
...
- 表格列 `RUN ID | 工作流 | 状态 | 进度 | 开始时间 | 需求`：状态徽标四态 running / completed / failed / interrupted（interrupted 为服务端按 pid 判活的**读时派生态**，与 `run show` 同逻辑）；进度列 running / interrupted 显示 `k/N`（interrupted 即中断时进度）、终态显示总步数；需求列表格内截断（与 CLI 人类表格同策略，**全文在详情页**）。进度分子 `k` 按 trace 中**唯一 `stepIndex` 且 `success`** 去重计（服务端 `store.CountProgress` → `run.ProgressCount`），而非物理行数——`run resume` 续跑过的 run 其 trace 含保留的失败行 + 补跑行、同一 `stepIndex` 有多条，去重保 `k ≤ N`（见 [cli-runtime.md](./cli-runtime.md)〈run resume〉的进度说明）。CLI `run list` 只输出总步数，不输出此 `k/N` 进度。
...
- **failed / interrupted（都可重跑）**：概要头提供**「重跑」**按钮（= `run resume <id>`，见 [cli-runtime.md](./cli-runtime.md)〈run resume〉）——`failed`（引擎报错中止）与 `interrupted`（进程被杀 / 崩溃）都可恢复，重入点统一由 trace 推断；确认弹窗后调用，跳过已成功步、从**中断步**续跑，**同一 run 续写、id 不变**，点击后该 run 转 `running`、刷新即见推进。两态展示略有差异：`failed` 把 `error` 全文置顶、**中断步（由 trace 末条 `success:false` 记录的 stepIndex 推断，非 run.json 字段）高亮标红**；`interrupted` 如实标注「进程已不在，尽力展示已有 trace」。续跑后**逐步列表**保留旧 trace 作审计：若同一 `stepIndex` 先有失败记录、后又有重跑记录，则旧失败行标注「已重跑取代」；`interrupted` 的中断步通常没有旧 trace 行（进程在写该步 trace 前退出），不承诺一定出现双行。只有 `completed`（无可恢复）与 `running`（存活、仍在跑）不显示重跑按钮。
- **逐步列表**：含循环（评测内循环 / 回跳）的运行按「第 N 轮」分组头聚合各轮迭代步，一眼看出循环了几轮；每步一行——step 序号 · 节点 id chip · 步骤名（复用 StepLabel：evaluator 步带「· 评测」后缀）· 引擎（icon + 引擎名）· 成功/失败 · 耗时 · tokens · 产物预览（单行截断）；**engineConfig 声明值（model / effort）不占行内列，只在展开详情顶部展示**；若该步引擎回报了会话 id，紧接其下再附一行**会话 id + 回放命令**（claude-code→`claude -r`、codex→`codex resume`、qoder→`qodercli -r`、antigravity→`agy --conversation`，未知引擎只显 id；与 `run show --trace` 一致），旁附复制按钮一键复制回放命令。**每步均可点击展开完整「输入 / 产物 / 错误」全文**（`--trace` 深度；以 **md 源码高亮**只读呈现，与提示词编辑器同款外观、不含占位符高亮），各段标题旁**复制按钮**一键复制全文。展开才构建 DOM（trace 单行可达 MB 级，全量直灌会卡死页面）——**延迟渲染是唯一允许的性能手段，点开必须完整全文**。
...
> **重跑（`run resume`）走同一发射器（已实装）**：运行详情页「重跑」经 `POST /api/runs/{id}/resume` 同样 self-exec 分离子进程（`conduct run resume <id>`）；`internal/launch` 已泛化为可发射 `workflow run` 与 `run resume`（`LaunchResume`）。因续写原 run，run id 即入参 `<id>`、**无需**上表的轮询匹配一步（改以「run.json 的 pid 被改成子进程 pid」确认接管），其余脱离 / 落盘 / pid 判活与 `workflow run` 发射逐字节一致。
...
1. **进度 k 按 `stepIndex` 去重、流式解析**：列表页为每个 running run 算进度分子 `k` 时用 `store.CountProgress`——逐行流式读、每行只解出 `stepIndex` / `success` 两字段（**不** materialize MB 级 input/output），按唯一 `stepIndex` 且末次 `success` 去重（防 resume 后 `k>N`，见〈run resume〉进度说明）；**禁止** `LoadTrace` 全量 JSON 解析只为算进度。数物理行数的 `store.CountTrace` 仅表达「已落盘多少条 trace」，不再作进度分子。
...
| `POST /api/workflows/{name}/copy` | `workflow copy <src> <dst>` | body `{newName}`；复制定义主体（nodes）为新托管对象、时间戳重戳不继承源；源不存在 404、目标占用 409、非法名 400、源语义非法 422；runs 分毫不动 |
...
| `GET /api/runs/{id}` | `run show <id> --json` | run.json 规范化内容（含 snapshot / artifacts / error 全文；无 failedStep 字段——失败步由 trace 推断） |
...
| `POST /api/runs/{id}/resume` | `conduct run resume <id>`（新增命令） | self-exec 分离子进程从中断步续跑（复用 `internal/launch`）；`failed` 或 `interrupted` 可恢复，`completed` / `running`（存活）→ 409；202 返回 `{runId}`（即原 id，同一 run 续写） |
| `GET /api/engines` | （无 CLI 命令：服务端直读能力表，只读信息性端点豁免） | 注册引擎 + 能力表（`allowsModel` / `effortField` / `effortValues` / `modelValues`），检查器引擎、effort 与 model 建议下拉的数据源；能力表缺失时以 `capability:null` 表达（不返回单独的 `ok` 字段） |
...
UI 自身（`internal/ui` 服务端 + 内嵌前端 + `internal/cli/ui.go` 注册命令）之外，以下配套能力与 UI 形成同一能力面；每条按当前代码状态标注：

1. **【服务端直读·无需新命令】`GET /api/engines` 直接读引擎能力表**——检查器引擎 / effort 下拉和 model 建议下拉的数据源，由 UI 服务端 handler 进程内组合既有导出 `engine.RegisteredNames()`（engine.go:77）+ `engine.Capability()` 生成，**不新增 `conduct engine list` 命令**（评审决策）。它是只读信息性端点，不产生任何 CLI 没有的副作用或独占能力，作为「无独占能力」不变量的显式豁免。`engine.Capability()` 返回 `ok=false`（已注册但能力表待实装）时用 `capability: null` 表达，不得误报成 `allowsModel:false`。
...
4. **【内核内部增强·已实装】`internal/store`：进度计数 + 读总结 + 全量读健壮化**——(a) 已新增进度计数：`CountProgress`（逐行流式解析、每行只取 `stepIndex` / `success` 两字段，按唯一 `stepIndex` 去重）供列表页为每个 running run 算进度 `k`，避免全量 `LoadTrace` 只为算进度（单行可达 MB 级）；配套 `CountTrace`（流式数 `\n`、零 JSON 解析）表达「已落盘多少条 trace」，不作进度分子——resume 后同一 `stepIndex` 有多条，数物理行会使 `k>N`；(b) 已新增 `ReadSummary`，供 `GET /api/runs/{id}/summary`，running 期尚未生成时返回哨兵 → 404；(c) `LoadTrace` 已从 `bufio.Scanner` 迁到按 `\n` 逐行读，消除超长行崩溃与半行假损坏——详情页刷新走的正是全量 `LoadTrace`。**不做**增量 tail 读取（改手动刷新后无 `since` 游标，见〈监控机制〉）。
...
6. **【新增命令·已实装，对齐规格】`conduct run resume <id>`**——从中断步恢复 `failed` / `interrupted` 两类未完成运行，规格见 [cli-runtime.md](./cli-runtime.md)〈run resume〉。为运行详情页「重跑」按钮提供 CLI 等价物（无独占能力）；对纯 CLI 用户同样是刚需。已落地：重入点由 trace 推断、放行 `failed` 与 `interrupted`、`failedStep` 字段已删（失败步改从 trace 末条 `success:false` 推断）；前端「重跑」按钮门控放宽到 `failed` 或 `interrupted` 都显示；`-d` 后台发射复用**泛化**后的 `internal/launch`（`LaunchResume`，run id 即入参、**无需 `matchRunID` 轮询**，有界等待以「run.json 的 pid 被改成子进程 pid」确认接管）。
7. **【命令增强·可选，不阻塞】`workflow run` 开跑即输出 run id**（人类模式首行 / `--json` 首行 start 事件）——UI 的 pid 组合匹配可退化为兜底；对纯 CLI 用户独立有价值（不必等运行结束就能另开终端 `run show` 跟进）。主方案零改动可用，此条可延后。
8. **【规格修订·随实现同步】`cli-tooling.md` / `cli-runtime.md`**——`cli-tooling.md`〈ui〉一节按本文〈命令形态〉定案待定项；`cli-runtime.md`〈workflow run〉回填 `--cwd` 存在性校验与「管道喂入空需求」的边界行为（当前只规定了 TTY 无参数退 2；UI 侧已双重拦截，CLI 侧行为值得明确）。

## 明确不做（与理由）

- **run 的删除按钮**：CLI 已有 `run rm`，但 UI 本阶段不暴露运行删除入口；运行历史仍以查询、终止、恢复为主，避免把不可逆删除放进监控面。（终止已随新增命令 `run stop` 进入 UI，见〈配套实现状态〉。）
