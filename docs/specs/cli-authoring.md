...
| 选项 | 类型 | 默认 | 说明 |
| --- | --- | --- | --- |
| `--engine <e>` | 字符串 | 必填 | 引擎，取值由 descriptor 注册表生成，见 [engines.md](./engines.md)〈引擎能力表〉 |
...
| `--effort <v>` | 字符串 | — | 推理档位；是否接受及合法枚举由所选引擎的 descriptor capability 决定 |
...
- `--reasoning-effort` 已删除，与 `--xxxabc` 一样按普通未知选项报错；退出 `2`，无专门迁移提示。
- 落盘副作用：仅校验通过时覆盖入库。
...
conduct workflow node set <name> <id> \
    [--id <newid>] [--engine <e>] [--model <m>] [--effort <v>] [--display-name <s>]
```
...
| `--engine <e>` | 字符串 | 设引擎，取值见〈workflow 定义 schema〉能力表。改 engine 可能使原 `engineConfig` 字段被新 engine 拒收 → 见下「诚实边界」 |
| `--model <m>` | 字符串 | 设模型；传空串 `--model ""` **清除**该字段（回落引擎默认模型） |
| `--effort <v>` | 字符串 | 设推理档位（是否接受及取值见 descriptor 能力表）；传空串清除 |
...
- `--reasoning-effort` 已删除，与任意其它未知选项走同一用法错误路径；退出 `2`。
- 落盘副作用：仅校验通过时覆盖入库。
...
- **清除标量用空串**：`--model` / `--effort` 用**空串**表达「清除、回落引擎默认」。空串本就是这些字段的非法值，语义无歧义。
- **`--engine` 改引擎的级联**：`engineConfig` 是随 `engine` 判别的载荷，改 engine 后旧的 `model` / `effort` 未必被新 engine 接受。此时若不同时清除不兼容配置，整份定义校验会失败、命令拒绝退 `1`——如切到 antigravity 时可在同一条命令给 `--engine antigravity --effort ""`。绝不静默丢弃不兼容字段。

**示例**：

```bash
conduct workflow node set flow gen  --model claude-sonnet-5        # 换模型
conduct workflow node set flow test --engine antigravity --effort ""  # 换引擎，同时清掉新引擎不接受的 effort
  ...
  "definition": {                 // 定义主体：edit / create --definition 的输入单元就是这个对象
    "nodes": [
      { "id": "START" },          // 保留标记节点：无 engine/prompt/engineConfig、不执行、不产 trace、无入边
      {
        ...
        "engine": "claude-code",  // 必填（判别式）；合法值来自 descriptor 注册表
        "engineConfig": {         // 选填；shape 由 engine 决定，合法字段见下「引擎载荷」
          "model": "claude-opus-4-8",  // 选填；取值受 engine 约束，省略则用引擎默认模型
          "effort": "high"        // 选填；是否接受及合法值由所选引擎的 descriptor capability 决定
        },
...
**engine + engineConfig（判别联合）**：`engine` 是判别式（tag），`engineConfig` 是引擎专属载荷，其合法字段由 `engine` 决定——这把「engine / model / effort 三者绑定」编进结构本身，三者不能各自独立填。支持的引擎、各引擎接受的 `engineConfig` 字段及枚举、schema 字段到 CLI 参数的映射，见 [engines.md](./engines.md)〈引擎能力表〉——那是引擎层的单一权威来源。`model` 省略则用该引擎默认模型。具体校验（严格、依能力表）见〈落盘校验规则〉。
...
- **engine + engineConfig 严格校验（判别联合）**：先按 `engine` 读取同一注册 `Descriptor` 的能力（见 [engines.md](./engines.md)〈引擎能力表〉），再校验 `engineConfig`：`engine` 合法（已注册）；`model` / `effort` 是否被该引擎接受；`effort` 是否在该引擎允许集内；任一不符即拒绝。`model` 当前**不做白名单**（接受任意非空串）；`ModelSuggestions` 仅供 UI 输入建议，不参与落盘校验。`reasoningEffort` 与 `xxxabc` 一样由严格 JSON 解码作为普通未知字段拒绝，不提供兼容或专门诊断。
...
校验失败时逐条打印字段级错误（如 `nodes[1].engineConfig.effort: engine="antigravity" 不接受 effort`、`edges: 检测到环 a→b→a`），退出码 `1`。
...
| `workflow node set` 瘦身 | **已实现**（`internal/cli/workflow_node_set.go` 已删除 `--evaluator` / `--no-evaluator` / `--redo-target` / `--no-redo` / `--loop-count`，留 `--id` / `--engine` / `--model` / `--effort` / `--display-name`；`--id` 走 `workflow.RenameNodeID` 级联改名） |
