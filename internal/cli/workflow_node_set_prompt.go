...
// 把某节点（或其 evaluator）的 promptTemplate 以原始文本从 stdin 读入，由 conduct 负责 JSON 编码，
...
func newWorkflowNodeSetPromptCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Short: "从 stdin 读原始文本设某节点 / 评测官的提示词（免手工 JSON 转义）",
		Long: "把提示词正文以原始文本从 stdin 整段读入，由 conduct 负责 JSON 编码——含 {{变量}} / 中文 / markdown / 多行都无需转义。\n" +
			...
			"落盘前复用整份定义的同一套校验（含空模板、模板变量引用）；stdin 是终端（无管道）时报错退出，不挂起等待输入。",
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if evaluator {
				if node.Evaluator == nil {
					return fmt.Errorf("节点 %s 无评测循环，无评测官提示词可设（先用 conduct workflow node set %s %s --evaluator --engine <e> 挂载）", id, name, id)
				}
				node.Evaluator.PromptTemplate = promptTemplate
			} else {
				node.PromptTemplate = promptTemplate
			}

			if err := workflow.Validate(def); err != nil {
				return err // 整份校验：空模板 / 模板变量引用非法在此退 1，原文件不变
			}
			...
			out := cmd.OutOrStdout()
			if evaluator {
				fmt.Fprintf(out, "✓ 已更新 %s·%s 评测官提示词\n", name, id)
			} else {
				fmt.Fprintf(out, "✓ 已更新 %s·%s 提示词\n", name, id)
			}
			return nil
		},
	}
	cmd.Flags().BoolVar(&evaluator, "evaluator", false, "设该节点评测官的提示词而非节点主体（节点无评测官时报错退 1）")
	return cmd
}
