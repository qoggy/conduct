...
func checkNodeSetFlagCombo(opts nodeSetOptions) error {
	if opts.NewID == nil && opts.Engine == nil && opts.Model == nil && opts.Effort == nil && opts.DisplayName == nil {
		return localizedUsageErrorf("至少给一个字段选项（--id / --engine / --model / --effort / --display-name）", "provide at least one field option (--id / --engine / --model / --effort / --display-name)")
	}
	return nil
}

// applyEngineConfig 把 opts 里给出的 model / effort 施加到 *carrier 指向的 EngineConfig 指针。
func applyEngineConfig(carrier **workflow.EngineConfig, opts nodeSetOptions) {
	if opts.Model == nil && opts.Effort == nil {
		return
	}
	config := *carrier
	if config == nil {
...
func newWorkflowNodeSetCommand() *cobra.Command {
	var newIDFlag, engineFlag, modelFlag, effortFlag, displayNameFlag string
	cmd := &cobra.Command{
		Use:   "set <name> <id>",
	...
	f.StringVar(&engineFlag, "engine", "", fmt.Sprintf(localizedHelpText("设引擎（%s）", "Set the engine (%s)"), engineNamesHelp()))
	...
	f.StringVar(&effortFlag, "effort", "", fmt.Sprintf(localizedHelpText("设推理档位（%s）；传空串清除", "Set the reasoning effort level (%s); pass an empty string to clear it"), effortValuesHelp()))
